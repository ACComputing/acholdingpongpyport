...
TITLE: Final[str] = "AC'S Pong // Neon Arcade"
...
class Game:
    ...
    def update_match(self, dt: float) -> None:
        ...
        if self.serve_timer > 0.0:
            ...
            # AI eases back to the middle during the serve countdown.
    ...
    def predict_ball_y(self) -> float:
        # Be forgiving when the ball moves away from the AI.
        if self.ball.vx < 0:
            return HEIGHT / 2
    ...
    def reflect_from_paddle(self, paddle: Paddle, *, direction: int) -> None:
        ...
        # Add a tiny bit of paddle movement influence to keep shots feeling responsive.
    ...
    def draw_menu(self, surface: pygame.Surface) -> None:
        self.draw_text_center(surface, "AC'S PONG", self.title_font, WHITE, WIDTH // 2, 132)
        self.draw_text_center(surface, "NEON ARCADE EDITION", self.tiny_font, CYAN, WIDTH // 2, 188)
        ...
        self.draw_text_center(
            surface,
            "UP / DOWN = Select   ENTER = Confirm   LEFT / RIGHT = Change Difficulty",
            self.tiny_font,
            GRAY,
            WIDTH // 2,
            HEIGHT - 52,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AC'S Pong // Neon Arcade")
